...
namespace phoenix {
    ...
    class number {
        public:
            ...
            number(const T &value, const int16_t &precision = 2) : _value(value), _precision(precision) {}

            T &value(void) { return _value; }
            const T &value(void) const { return _value; }
            void value(const T &value) { _value = value; }            

            void precision(int16_t precision) { _precision = precision; }
            const int16_t &precision(void) const & { return _precision; }

            inline std::string string(void) const { return (std::stringstream() << std::fixed << std::setprecision(precision()) << value()).str(); }
